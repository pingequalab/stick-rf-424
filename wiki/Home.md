# Stick RF 424 for M5StickC Plus — Wiki

> **[Buy Stick RF 424 — $24.98 USD](https://www.pingequa.com/products/stick-rf-424-module)** | Built by [PINGEQUA](https://www.pingequa.com) — *Precision Gear for Hackers*

---

## What is the Stick RF 424?

The **Stick RF 424** is a dual-band RF module for the **M5StickC Plus 1.1 and 2**. It integrates:

- **CC1101** — Sub-GHz radio (315 / 433 / 868 / 915 MHz)
- **nRF24L01+** — 2.4GHz transceiver

A **physical hardware slide switch** provides true electrical isolation between chipsets — no software conflicts, no firmware overhead. It is one of PINGEQUA's most popular products with **92+ community reviews**.

---

## Quick Navigation

| Topic | Wiki Page |
|-------|-----------|
| Hardware setup & firmware flash | [Setup Guide](Setup-Guide) |
| Bruce firmware version guide | [Firmware Guide](Firmware-Guide) |
| Common errors & fixes | [Troubleshooting](Troubleshooting) |
| Frequently asked questions | [FAQ](FAQ) |
| Official support page | [PINGEQUA Support](https://www.pingequa.com/pages/support-m5stack-stickc-plus-2-in-1-module) |

---

## Technical Summary

| Parameter | Value |
|-----------|-------|
| Sub-GHz Chipset | CC1101 |
| 2.4GHz Chipset | nRF24L01+ |
| Isolation Method | Physical slide switch |
| PCB Design | Low-noise with enhanced capacitance |
| Required Firmware | Bruce latest beta |
| Avoid | Bruce v1.13 stable (SPI bug) |
| Compatible Devices | M5StickC Plus 1.1 / M5StickC Plus 2 |
| Price | $24.98 USD |
| Reviews | 92+ |

---

## Switch Positions

| Switch Position | Active Module |
|-----------------|--------------|
| 433 MHz | CC1101 (Sub-GHz) |
| 2.4 GHz | nRF24L01+ |

---

## Related Products

| Product | Device |
|---------|--------|
| Stick RF 424 | M5StickC Plus 1.1 / 2 |
| [RF PACK S3](https://github.com/pingequalab/rf-pack-s3) | M5Stack StickS3 |
| Hydra RF 424 Cap | M5Stack Cardputer |
| SWITCHBLADE RF 3-in-1 | Flipper Zero |

Full lineup at [pingequa.com/pages/wiki](https://www.pingequa.com/pages/wiki)

---

> *For educational and authorized research purposes only.*
